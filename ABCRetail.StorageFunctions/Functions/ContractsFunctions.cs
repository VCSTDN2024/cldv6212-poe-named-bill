...
public sealed class ContractsFunctions
{
    ...
    private readonly ContractsFileService _contractsFileService;
    private readonly ILogger<ContractsFunctions> _logger;

    public ContractsFunctions(ContractsFileService contractsFileService, ILogger<ContractsFunctions> logger)
    {
        _contractsFileService = contractsFileService;
        _logger = logger;
    }

    [Function("ContractsUpload")]
    public async Task<HttpResponseData> UploadAsync(
        [HttpTrigger(AuthorizationLevel.Function, "post", Route = "contracts")] HttpRequestData request)
    {
        var cancellationToken = request.FunctionContext.CancellationToken;
        var payload = await JsonSerializer.DeserializeAsync<ContractUploadRequest>(request.Body, SerializerOptions, cancellationToken);
        ...
        {
            _logger.LogWarning(ex, "Failed to decode base64 contract payload for {FileName}", payload.FileName);
        ...
        await _contractsFileService.UploadContractAsync(payload.FileName, stream, cancellationToken);

        _logger.LogInformation("Contract {FileName} uploaded with {Length} bytes", payload.FileName, bytes.Length);
    ...
    private sealed class ContractUploadRequest
    {
        [Required]
